	...
	<div class="content">
		...
		<div class="ptrang">
			<?php 
				...
				$name=$_GET['name'];
				$product_mn = $product->getallproduct_mn($name);
				$product_count = mysqli_num_rows($product_mn);
				$product_num = ceil($product_count/8);
				$current_page = !empty($_GET['pagemn'])?$_GET['pagemn']:1;
				$i=0;
				if($current_page>3){
					echo '<a class="phantrang" href="dongho.php?name='.$name.'&pagemn=1">'.'First</a>';
				}
				if($current_page>1){
					echo '<a class="phantrang" href="dongho.php?name='.$name.'&pagemn='.($current_page-1).' ">'.'<'.'</a>';
				}
				for($i=1; $i<=$product_num; $i++){
					if($i != $current_page){
						if($i>$current_page-3 && $i<$current_page+3){
							echo '<a class="phantrang" href="dongho.php?name='.$name.'&pagemn='.$i.' ">'.$i.'</a>';
						}
					}else{
				...
				if($current_page<$product_num-3){
					echo '<a class="phantrang" href="dongho.php?name='.$name.'&pagemn='.$product_num.'">'.'Last</a>';
				}
			?>
		</div>
      	<div class="section group">
      		<?php
      			$name=$_GET['name'];
      			$product_dh = $product->getproduct_dh($name);
      			if($product_dh){
